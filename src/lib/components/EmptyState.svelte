	...
	<div class="flex flex-col items-center gap-3 sm:flex-row">
		{#if actionLabel && onAction}
			<button
				type="button"
				onclick={onAction}
				class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-xs font-bold text-white shadow-primary/20 shadow-md transition-all duration-180 hover:bg-primary-hover focus:outline-none focus-visible:ring-2 focus-visible:ring-primary active:scale-[0.98]"
			>
				{actionLabel}
			</button>
		{/if}
