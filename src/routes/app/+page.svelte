<script lang="ts">
	...
	const fetchDueReviews = async () => {
		try {
			const { data } = await apiFetch<{ count?: number }>('/api/spaced-repetition/due');
	...
	let sharingCourseId = $state<string | null>(null);

	const handleOpenShare = async (courseId: string) => {
		if (!courseId || sharingCourseId) return;
		sharingCourseId = courseId;
		try {
			...
			toastStore.error(err instanceof Error ? err.message : 'Could not generate share link');
		} finally {
	...
	const handleForkCourse = async (courseId: string) => {
		...
		try {
			const { data } = await apiFetch<{ courseId: string }>(`/api/courses/${courseId}/fork`, {
				method: 'POST'
			});
				...
				<div class="flex flex-col justify-between gap-6 md:flex-row md:items-center">
					...
					<div class="flex shrink-0 flex-col items-stretch gap-3 sm:flex-row md:flex-col">
						<a
							...
							class="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-3.5 text-xs font-bold text-white shadow-primary/20 shadow-md transition-all duration-180 hover:scale-[1.02] hover:bg-primary-hover active:scale-95"
						>
