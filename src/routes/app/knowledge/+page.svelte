<script lang="ts">
	...
	async function handleSummarize() {
		...
		summaryResult = '';
		try {
			const { data } = await apiFetch<{ summary: string; provider?: string }>('/api/summarize', {
				method: 'POST',
				body: { text: summarizeText }
			});
	...
	async function handleParaphrase() {
		...
		paraphraseResult = '';
		try {
			const { data } = await apiFetch<{ paraphrase: string; provider?: string }>(
				'/api/paraphrase',
				{
					method: 'POST',
					body: { text: paraphraseText, style: paraphraseStyle }
				}
			);
	...
	async function handleGenerateFlashcards() {
		...
		generatedCards = [];
		try {
			const { data } = await apiFetch<{ flashcards?: Array<{ front: string; back: string }> }>(
				'/api/documents/flashcards',
				{
					method: 'POST',
					body: { documentText: flashcardText }
				}
			);
	...
	async function loadStats() {
		statsLoading = true;
		try {
			const { data } = await apiFetch<{ chunk_count?: number; has_documents?: boolean }>(
				'/api/documents'
			);
	...
	async function uploadAll() {
		...
		uploadError = '';
		try {
			const { data } = await apiFetch<{ chunks_added: number }>('/api/documents', {
				method: 'POST',
				body: { texts: stagedFiles.map((f) => f.content) }
			});
			toastStore.success(
		...
		{#if stagedFiles.length > 0}
			<div class="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-6 shadow-sm">
				...
				<button
					...
					onclick={uploadAll}
					disabled={uploading}
					class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-xs font-bold text-white shadow-primary/20 shadow-md transition-all hover:bg-primary-hover active:scale-[0.98] disabled:opacity-50"
				>
					{#if uploading}
						<span
